...
#5. Plot the data
Counties_sf_plot <-ggplot(data=Counties_sf) +
  geom_sf()
...
> ANSWER: The EPSG code of the Counties dataset is 4269. This uses
> degrees which are angular units so this is a geographic coordinate
> reference system. This is associated to the
...
NWIS_Gage_Location<-read.csv(here("Data/Raw/NWIS_SiteInfo_NE_RAW.csv"),
...
9.  What columns in the dataset contain the x and y coordinate values,
    respectively?\
    \> ANSWER: "dec_long_va" contains the x-coordinates, or longitude
    values. "dec_lat_va" contains the y-coordinates, or latitude values.
    \>
...
NWIS.gage.sf <- NWIS_Gage_Location %>% 
...
> ANSWER: The geometry column ("geometry") has appeared. The latitude
> and longitude columns ("dec_lat_va" and "dec_long_va") dissappeared.
...
flow_gage_join<-merge(x=NWIS.gage.sf,
                      y=NWIS_SiteFlow_df,
                      by='site_no',
                      all=TRUE) %>% 
  na.omit()
...
NWIS.gage.flow_plot <-ggplot()+
  geom_sf(data=Counties_sf)+ 
  geom_sf(data=flow_gage_join,
          aes(color=gage_ht))
NWIS.gage.flow_plot
...
Gage_utm<-st_transform(NWIS.gage.sf, crs = 32614)


#21 Plot the data
Counties_Gage_Utm_Plot<-mapview(Gage_utm)+
  mapview(Counties_utm,
          col.regions = 'orchid')

Counties_Gage_Utm_Plot
```
...
24. Create a plot showing: \* all
Nebraska counties, \* the selected county, \* and the gage sites in that
county

```{r spatial selection}
#22 Select the county
Dawson_utm<-Counties_utm %>%
  filter(NAME=="Dawson")

#23 Spatially select gages within the selected county
Dawson_gages <- Gage_utm[Dawson_utm,]

#24 Plot
Dawson_plot<-mapview(Dawson_gages)+
  mapview(Dawson_utm,
          col.regions='slateblue')+
  mapview(Counties_utm,
          col.regions='orchid')
