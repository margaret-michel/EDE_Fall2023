...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #Parse "month" from "average"
...
#To verify if there are dates missing that are not in the tables, create a sequence of dates with from=first date, to=last date, time step = frequency of observations > left join sequence of dates with data sets and then R will populate the blank dates with NAs. Then do a summary to see how many NAs
...
Rainfall <-ggplot(Rainfall_BR_processed, aes(x = Date, y = Rainfall)) +
  geom_line()+
  geom_smooth()

Rainfall #No trend - the blue trend line is just flat over time but it does go up and down fast which is an indication of seasonality

Temperature <-ggplot(Temp_BR_processed, aes(x = Date, y = Temperature_C)) +
  geom_line()+
  geom_smooth()
...
#TS objects will be stored as vectors under Environment > Values
```
...
BR_Rain_Components <- as.data.frame (BR_Rain_ts)
